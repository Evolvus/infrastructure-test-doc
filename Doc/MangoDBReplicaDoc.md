...
Copy and paste it in this path “/etc/init.d/disable-transparent-hugepages”.
...
Add below parameters in  “ /etc/tuned/no-thp/tuned.conf “  File.
...
Create a keyfile
Create a key file in one of your instance by following command.

#openssl rand -base64 756 > /var/lib/mongo/keyfile 
(mongodb user home directory path may be vary in different OS. Check the actual path in “/etc/passwd” file)
#chmod 400 /var/lib/mongo/keyfile 
#chown mongod:mongod /var/lib/mongo/keyfile

Copy keyfile to each replica set member in same location 
...
Step 9: Next create replica set and run the mongodb from command line and initiate the nodes.

#mongo -u admin -p --authenticationDatabase admin

> config = {_id: "rep1", members:[
{_id: 0, host: '10.0.1.147:27017'},
{_id: 1, host: '10.0.1.118:27017'}]
}
